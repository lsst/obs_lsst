...
# PTC in butler query-collections /sdf/group/rubin/repo/main u/snyder18/*crosstalk_analysis*
...
for det_id, detector in enumerate(camera):
    ...
    if det_name in ['R00_SW0', 'R00_SW1', 'R04_SW0', 'R04_SW1', 'R40_SW0', 'R40_SW1', 'R44_SW0', 'R44_SW1']:
        nAmp = 8
    ...
    where = "instrument='LSSTCam'" + \
            " and detector={0}".format(det_id)

    data_refs = list(registry.queryDatasets('crosstalkResults',
        ...
        for j in range(nAmp):
            ...
            for k in range(nAmp):
                ...
                if j == k:
                    ...
                    continue
                try:
                    c0_matrix[k, j, i] = crosstalk_results[det_name][det_name][target_segment][source_segment]['c0']
                    c1_matrix[k, j, i] = crosstalk_results[det_name][det_name][target_segment][source_segment]['c1']
                    c0_error[k, j, i] = crosstalk_results[det_name][det_name][target_segment][source_segment]['c0Error']
                    c1_error[k, j, i] = crosstalk_results[det_name][det_name][target_segment][source_segment]['c1Error']
                except KeyError:
                    continue
    ...
    out_path = os.path.join(directory, 'lsstCam',
                            "crosstalk", det_name.lower())
    os.makedirs(out_path, exist_ok=True)
    out_file = os.path.join(out_path, datestr + ".ecsv")

    cc.updateMetadata(camera=camera, detector=detector,
                       setCalibId=True, setCalibInfo=True,
                       setDate=True)
